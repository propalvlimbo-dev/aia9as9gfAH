...
- **Таб** у неавторизованного показывает только команды авторизации,
...
| `session.max.seconds` | `43200` (12 ч) | срок жизни сессии |
...
| `/addtg` | получить код привязки Telegram (работает на любом сервере сети); код живёт 5 минут, повторный `/addtg` новый код не выдаёт, пока старый жив |
...
### Админ-команды (`/elytrixauth`, алиас `/ea`; право `elytrixauth.admin` или `*`/`elytrixauth.*`, консоль — всегда)
