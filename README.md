...
| `/addtg` | получить код привязки Telegram (работает на любом сервере сети); код живёт 5 минут, повторный `/addtg` новый код не выдаёт, пока старый жив. В чате — кликабельные `/link <код>` (клик копирует) и `@Elytrixbot` (открывает бота), над хотбаром — процесс с отсчётом до удаления кода, после привязки — баннер «Вы успешно привязали аккаунт к боту» ~15 сек |
